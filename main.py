...
def compute_bs(df, alpha, beta, gamma, rho_s):
    u = (
        alpha * df["A"].to_numpy() * df["Y"].to_numpy()
        + beta * df["R"].to_numpy()
        - gamma * df["iota"].to_numpy() * df["c"].to_numpy()
    )
    c = df["c"].to_numpy()
    t = df["t"].to_numpy()
...
def main():
    ap = argparse.ArgumentParser(
        description="Compute Bitcoin-Seconds index from CSV time series."
    )
    ap.add_argument(
        "--input", required=True, help="CSV with columns: timestamp,A,Y,R,c,iota"
    )
    ...
    rho_s = (
        args.rho_per_second
        if args.rho_per_second is not None
        else args.rho_per_year / SEC_PER_YEAR
    )
